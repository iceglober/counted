...
export const AGENT_PROMPT = `You are adding privacy-first product analytics to this project using Counted (counted.dev). Counted uses no cookies, no fingerprinting, and no PII — keep it that way. Work step by step and don't skip steps.
...
3. Read the integration guide at https://counted.dev/docs/llms.txt for the exact SDK install command, the init snippet, and the event/property format for this project's language (JS, React, Python, Go, or Rust).
