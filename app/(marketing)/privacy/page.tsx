...
export default function PrivacyPage() {
  return (
    <LegalPage title="Privacy Policy" updated="June 3, 2026">
      ...
      <P><strong>Analytics events you send.</strong> When you instrument your app with a Counted SDK, we receive the event names and properties <em>you</em> choose to send, plus coarse system properties (such as OS name/version, locale, app version, and device model where your SDK provides them) and an ephemeral session identifier. We do not store IP addresses or attach them to your events. Like any web service, our servers receive the IP that sends a request; we use it transiently — held in memory for seconds — only to rate-limit abuse, and it is never written to disk, logged, or linked to your analytics. For a coarse country breakdown we read only a two-letter country code that our edge network derives from the request, then discard — no IP is stored, and there is no city, region, or precise location. You control what properties you send — please don&apos;t send personal data in event properties.</P>
