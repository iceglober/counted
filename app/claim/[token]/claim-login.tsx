...
export function ClaimLogin({ token, socialEnabled }: { token: string; socialEnabled?: boolean }) {
  ...
  async function social(provider: "github" | "google") {
    ...
    try {
      ...
      if (result?.error) setError(result.error.message ?? `Couldn't sign in with ${provider}`);
    } catch (err) {
      setError(err instanceof Error ? err.message : `Couldn't sign in with ${provider}`);
    }
  }
