 ...
 * (state, event) to state means every transition is a test rather than a Stripe
 * account and a card.
 *
 ...
 *
 * Here a transition always produces a subscription, and
 * `SubscriptionRepository.save` is an upsert with no update-shaped sibling. The
 * shape that can match nothing does not exist.
 */

import { assertNever, type Instant, type WorkspaceId } from "@counted/kernel";
import { Entitlement, type PaymentState } from "./entitlement";
import type { PlanId } from "./plan";
import type { WorkspaceStanding } from "./workspace";

/** The provider's ids, kept opaque. The domain never parses or constructs one. */
...
export const Subscription = {
  /** What every workspace starts as. Free, unpaid, entitled to the free plan. */
  ...
  /**
   * The two facts the Workspace needs. Naming the projection stops a caller
   * from copying `plan` across and forgetting `payment`, which is precisely how
   * a canceled subscription kept its paid limits in v1.
   */
  standingOf: (s: Subscription): WorkspaceStanding => ({ plan: s.plan, payment: s.payment }),

  /** Whether the provider can be asked to open a billing portal for this workspace. */
  hasBillingAccount: (s: Subscription): boolean => s.customer !== null,
} as const;
 ...
 * provider event is the adapter's problem, and an unrecognised one is ignored
...
export type BillingEvent =
  ...
  | {
      ...
      readonly subscription: SubscriptionRef;
      readonly renewsAt: Instant | null;
      /** The provider's status, already narrowed by the adapter. */
...
export const applyBillingEvent = (
  ...
  return {
    subscription: next,
    entitlementChanged: !Entitlement.equal(before, after),
