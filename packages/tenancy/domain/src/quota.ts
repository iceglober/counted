 ...
 * for byte the same response as success. A customer could be losing every event
 * and see nothing but 202s.
 *
 ...
 *
 * **The three names are the wire's names.** `ok` / `overage` / `rejected` is
 * what the usage response already publishes, so this union uses them verbatim
 * rather than v2's `accept` / `overage` / `reject` plus a translation at the
 * boundary. A translation table between two vocabularies for one concept is a
 * place for them to drift; there is no such table now.
 */
 ...
 * entitlement and nowhere else — an earlier draft let the caller pass one, and
 * `period.limit ?? entitlement.limits.eventsPerMonth` then read an explicit
 * `null` (meaning unlimited) as "not supplied" and silently applied the plan's
 * cap instead. Two sources for one number is how v1 ended up with three
 * definitions of "is this customer on Pro?".
 */
export type UsagePeriod = { readonly used: number };

export type QuotaState = "ok" | "overage" | "rejected";

export type QuotaDecision =
  | { readonly kind: "ok"; readonly used: number; readonly limit: number | null }
  | {
...
export const Quota = {
  ...
  decide: (entitlement: Entitlement, period: UsagePeriod): QuotaDecision => {
    ...
    if (limit === null) return { kind: "ok", used: period.used, limit: null };

    if (period.used < limit) return { kind: "ok", used: period.used, limit };

    // A zero allowance is past by any amount, and dividing by it would report
    // NaN rather than "over".
    const ratio = limit === 0 ? Number.POSITIVE_INFINITY : period.used / limit;
    if (ratio < OVERAGE_MULTIPLE) return { kind: "overage", used: period.used, limit, ratio };
    return { kind: "rejected", used: period.used, limit, ratio };
  },

  /** Whether events are still being stored. Overage counts as accepted. */
  accepts: (d: QuotaDecision): boolean => d.kind !== "rejected",

  /** Whether the customer should be told something is wrong. */
  needsAttention: (d: QuotaDecision): boolean => d.kind !== "ok",
