...
Create a project in [the app](https://app.counted.dev) under **Projects → New project**. The setup code includes its first ingest key.
...
Create one client per app. The browser sends queued events automatically; watch **Overview → Live events** in your project to confirm they arrive.
...
Distinct-person/account queries and cross-visit retention are not available yet. Do not collect IDs solely for an unavailable analysis. Numeric event properties can be filtered or grouped, but cannot be summed unless the project schema lists them as declared measures.

If you explicitly use `identify()`, call `reset()` when a person signs out. In scripts and serverless handlers, `await counted.shutdown()` flushes remaining events and stops timers. `onDiagnostic` reports quota, rejected events, invalid credentials, and queue overflow.
