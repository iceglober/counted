 ...
 *
 * This type is also the reason v1's `emptyData()` cannot come back. A `Result`
 * has no zero value: there is no way to "render nothing" without first
 * deciding, in code, that you are looking at an `Err`.
 */
 ...
 * order. Use when any single failure fails the whole.
 */
 ...
 * partial outcome is meaningful — an ingest batch, where some events are
 * accepted and some rejected, is the case this exists for.
 */
