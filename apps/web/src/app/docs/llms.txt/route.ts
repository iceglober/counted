...
Any other language (no SDK yet — POST the wire contract below directly):
    # e.g. Python with requests/httpx: POST /v1/events with the
...
POST ${API}/v1/events
  ...
  Body: { "events": [{ "name": "...", "visitId": "...", "occurredAt": "<ISO8601>",
          "properties": { ... }, "systemProperties": { "os_name": "...", "sdk_version": "..." } }] }
  -> 202 with a receipt naming what was accepted, deduplicated and refused.
     A 202 is not a blanket success: read the receipt.
