...
export default async function ConsentPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const me = requireAccount(await attempt((await clientForCaller()).account.me({})), `/consent?${query}`);
  return <CenteredPage><Card><CardHeader><h1 className="font-heading text-xl">Connect an application</h1></CardHeader><CardContent><OAuthConsent query={query} email={me.account.email} /></CardContent></Card></CenteredPage>;
}
