...
export function CodeTabs({ tabs }: { tabs: CodeTab[] }) {
  ...
  function choose(i: number) {
    // Read once, so the persist and the broadcast cannot disagree about
    // which tab was chosen.
    const chosen = tabs[i];
    setActive(i);
    try {
      if (chosen !== undefined) localStorage.setItem(STORAGE_KEY, chosen.lang);
    } catch {
      /* ignore */
    }
    if (chosen !== undefined) {
      window.dispatchEvent(new CustomEvent(SYNC_EVENT, { detail: chosen.lang }));
    }
  }
